...
e_arr = 0.4*2**(-1*np.arange(1,10,1,dtype=float))
leg_nvar = 2**np.arange(5,10)
dat = MyLegQuadrature(lambda x: delta_sin(x,100),-1e5,1e5,leg_nvar,m=int(2e3))
...
def get_fi(f,rtol):
    ...
    for i,e in enumerate(e_arr):
        ...
        #dat[i] = MyLegQuadrature(lambda x: f(x,e_arr[i]),-1e5,1e5,leg_nvar,m=int(2e3))
        
        if i == 0 :
            continue
        if abs(dat[i-1][1]-dat[i][1]) <= rtol*abs(dat[i][1]):
    ...
    plt.plot(herm_nvar,dat[0],label= f"err_",marker ="1")
    return(dat)
